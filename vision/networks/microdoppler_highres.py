...
class EncoderVq_highres(nn.Module):
    def __init__(self, dim_z, cfgs, flg_bn=True, flg_var_q=False):
        super(EncoderVq_highres, self).__init__()
...
class DecoderVq_highres(nn.Module):
    def __init__(self, dim_z, cfgs, flg_bn=True):
        super(DecoderVq_highres, self).__init__()
        # Resblocks
        num_rb = cfgs.num_rb
        layers_resblocks = []
        for i in range(num_rb-1):
...
class EncoderVq_highres_resnet(EncoderVq_highres):
    def __init__(self, dim_z, cfgs, flg_bn, flg_var_q):
        super(EncoderVq_highres_resnet, self).__init__(dim_z, cfgs, flg_bn, flg_var_q)
        self.dataset = "MicroDoppler"


class DecoderVq_highres_resnet(DecoderVq_highres):
    def __init__(self, dim_z, cfgs, flg_bn):
        super(DecoderVq_highres_resnet, self).__init__(dim_z, cfgs, flg_bn)
        self.dataset = "MicroDoppler" 